...
subida) é calculada e usada para as curvas, mas o sombreamento das 3 fases não é
desenhado nos gráficos de comparação (ficaria conflitante com 4 pessoas/regiões
diferentes) — em vez disso, marcamos com linhas verticais o início da descida e o
vale médio de cada pessoa.
...
st.caption(
    ...
    "Sombra = ±1 desvio padrão entre os ciclos daquela pessoa/região. Linhas verticais "
    "pontilhadas (na cor da pessoa) marcam o início médio da descida e o vale médio de "
    "cada pessoa. Cada painel já dá pra ler nos dois sentidos: A×B (compare as cores) e "
    ...
    for col_i, direction in enumerate(DIRECTIONS, start=1):
        for ctx in PERSON_CTXS:
            for region in REGIONS:
                ...
                fig.add_vline(x=ctx["d_frac"], line_dash="dot", line_color=color, opacity=0.45, row=row_i, col=col_i)
                fig.add_vline(x=ctx["v_frac"], line_dash="dot", line_color=color, opacity=0.45, row=row_i, col=col_i)
...
st.divider()
st.caption(
    "Próximos passos possíveis: segmentar a comparação por fase (platô/descida/subida) "
    "separadamente em vez de só linhas verticais, comparação trial a trial (não só a "
    "resultante), e uma tabela de métricas (pico, CV, assimetria) por pessoa/região."
)
