...
def find_stable_plateau(series: np.ndarray | None, x_axis: np.ndarray,
                        search_start: float | None = None,
                        min_duration: float = 3.0, tol_frac: float = 0.08,
                        win_seconds: float = 1.0) -> tuple | None:
    """
    ...
    automaticamente uma janela de calibração (ex.: joelho mantido num
    ângulo conhecido por alguns segundos), sem precisar que o usuário
    digite os instantes na mão.

    tol_frac: variação máxima tolerada dentro de uma janela deslizante de
    win_seconds, como fração da amplitude TOTAL da série (a partir de
    search_start) — se adapta à escala de cada sinal, em vez de um valor
    fixo em graus.
    min_duration: duração mínima (s) do platô pra ser considerado válido.

    Retorna (t_start, t_end) do platô mais longo encontrado, ou None se
    não achar nenhum trecho estável o bastante.
    """
    if series is None:
        return None
    n = min(len(series), len(x_axis))
    mask_search = np.ones(n, dtype=bool) if search_start is None else (x_axis[:n] >= search_start)
    ...
    fs_local = 1.0 / np.median(np.diff(x)) if len(x) > 1 else 100.0
    win = max(3, int(win_seconds * fs_local))
    total_range = float(np.nanmax(y[valid]) - np.nanmin(y[valid]))
    if total_range <= 0:
        return None
    tol = tol_frac * total_range
